...
## Increment 13 (implemented)
- Added explicit `NotFoundException` domain exception and API mapping to HTTP 404 (`NOT_FOUND` error code).
- Updated media flows to use not-found semantics for missing products/photos instead of generic bad-request responses.
